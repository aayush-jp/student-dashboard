...
export async function updateSession(request: NextRequest) {
  ...
  if (user && !isAuthPage && !isOnboardingPage) {
    // Fetch user profile to check if they have selected a domain
    ...
    // Redirect to onboarding if no domain selected
    if (!profile?.selected_domain_id) {
      const url = request.nextUrl.clone()
      url.pathname = '/onboarding'
      const newResponse = NextResponse.redirect(url)
      // Copy cookies from supabaseResponse to newResponse
